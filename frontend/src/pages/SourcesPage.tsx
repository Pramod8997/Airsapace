...
const POLICY_META: Record<string, { cls: string; short: string; legend: string }> = {
  ...
  RESTRICTED_DOCUMENTED: { cls: 'border border-warning/50 bg-warning/10 text-warning', short: 'Restricted', legend: 'PS-named portal whose robots.txt and/or terms of service restrict automated access — documented, never collected (evidence: research_sources.md + saved robots.txt)' },
  REJECTED_ROBOTS_OR_TOS: { cls: 'border border-critical/50 bg-critical/10 text-critical', short: 'Rejected', legend: 'robots.txt disallow or ToS bot ban covers exactly the fare-search paths we would need — rejected, never worked around' },
